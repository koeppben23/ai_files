"""Canonical runtime deterministic pack lock builder."""
    ...
    for pack_id in resolved_ids:
        ...
        for surface in manifest.owns_surfaces:
            ...
            if existing_owner is not None and existing_owner != pack_id:
                raise ValueError(f"surface conflict detected: {pack_id!r} and {existing_owner!r} both own {surface!r}")
    ...
    for pack_id in resolved_ids:
        ...
        for surface in manifest.touches_surfaces:
            ...
            if owner not in manifest.requires:
                raise ValueError(f"surface conflict detected: {pack_id!r} touches {surface!r} but does not require owner {owner!r}")
