...
from governance_runtime.entrypoints import schema_selfcheck
...
def test_reason_payload_registry_exists_at_expected_path():
    ...
    assert (root / "governance_runtime" / "assets" / "catalogs" / "reason_codes.registry.json").exists()


@pytest.mark.governance
def test_embedded_reason_registry_matches_governance_registry():
    root = Path(__file__).resolve().parents[1]
    registry_path = root / "governance_runtime" / "assets" / "catalogs" / "reason_codes.registry.json"
