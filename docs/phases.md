...
| Phase 4 - Ticket Execution (planning) | Produces the concrete implementation plan and review artifacts; no code output yet. | Planning phase; code-producing output remains blocked until explicit gate progression permits it. Supports a review-only rail via `/review` for lead/staff depth feedback. |
